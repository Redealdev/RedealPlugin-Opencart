...
class ControllerModuleRedeal extends Controller {

    private $error = array(); // This is used to set the errors, if any.

    public function index() {   // Default function 
        $this->language->load('module/redeal'); // Loading the language file of redeal 

        $this->document->setTitle($this->language->get('heading_title')); // Set the title of the page to the heading title in the Language file i.e., redeal
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { // Start If: Validates and check if data is coming by save (POST) method
            $this->model_setting_setting->editSetting('redeal', $this->request->post);      // Parse all the coming data to Setting Model to save it in database.
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/redeal', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );

        /* End Breadcrumb Block */

        $this->data['action'] = $this->url->link('module/redeal', 'token=' . $this->session->data['token'], 'SSL'); // URL to be directed when the save button is pressed
        ...
        /* This block checks, if the redeal checkbox and GTM ID field is set it parses it to view */

        if (isset($this->request->post['redeal_enable_disable_field'])) {
            echo $this->data['redeal_enable_disable_field'] = $this->request->post['redeal_enable_disable_field'];
        } else {
       ...
       /* if (isset($this->request->post['redeal_text_gtm_id'])) {
        ...
        if (isset($this->request->post['redeal_module'])) {
            $this->data['modules'] = $this->request->post['redeal_module'];
        } elseif ($this->config->get('redeal_module')) {
            $this->data['modules'] = $this->config->get('redeal_module');
        }
        /* End Block */
        ...
        $this->template = 'module/redeal.tpl'; // Loading the redeal.tpl template
        $this->children = array(
            'common/header',
            'common/footer'
        );  // Adding children to our default template i.e., redeal.tpl 
    ...
    protected function validate() {
        ...
        if (!$this->user->hasPermission('modify', 'module/redeal')) {
